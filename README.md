...
TRG 无线温度标签（TTAG）全流程工具：基站监听 → 水浴控制 → 多设备自动标定 → 多项式拟合 → 复测验证 → 精度分析。

## 快速开始

```bash
# GUI（推荐）
python ttag_cal_app.py          # 或双击 run_ttag_gui.bat

# 命令行复测
python ttag_verify.py --device 201154
...
- **标定模式**：添加设备号 → 设温度范围 → 开始。多设备同时标定，每设备独立 Sheet + CSV 备份。
- **验证模式**：加设备 + 设协议 + 设测量间隔 → 设温度点 → 开始。实时表格 + Excel 输出。
- **推一把**：温度卡住时点「⚡ 推一把」，再点取消。
- **底部数据表格**：仅验证模式可见，实时显示每设备误差。

## 硬件

| 组件 | 型号 / 规格 |
|------|-------------|
| 温度标签 | TTAG 无线标签 T100-230 (ADC, NTC) / T100-316 (直接温度, 新协议) |
...
| 协议 | 标签型号 | 标签块大小 | 温度获取方式 |
|------|----------|-----------|-------------|
| **旧协议** | T100-230 | 9 字节/标签 | ADC 值 → 多项式 → 温度 |
| **新协议** | T100-316 | 8 字节/标签 | 标签直接报温度 (raw_temp ÷ 10 = °C) |

> 检测方式：`data_len - 5 == 1 + 8 × N` → 新协议；`== 9 × N` → 旧协议。
...
验证标签温度精度——将标签读数与水浴实际温度对比，误差须在 ±1°C 以内。

**适用协议：** T100-316（新协议）和 T100-230（旧协议），程序自动识别。
...
python ttag_verify.py --bath-tolerance 0.3  # 放宽水浴容差

# 端口被占用时自动切换客户端模式（连接基站 192.168.3.188:20226）
```

**温度点输入：**
- 逐个列举：`-18, -10, 0, 20, 50, 80`
- 范围+步长：`5 90 5`（5→10→15→...→90，推荐升温方向）
- 降序范围：`90 5 5`（支持倒序，自动从低到高排列）
- 预设分组：`low` / `mid` / `high`

**运行流程（全自动）：**
```
① 设定水浴 → ② 等待稳定 (容差 ±0.3°C) → ③ 等待标签数据稳定 (≥5 样本)
→ ④ 温度对比 → ⑤ 立即写入 Excel → ⑥ 3 秒后自动继续下一个点
```

**特性：**
- **点对点保存**：每点测完立即写 Excel，崩溃/断电不丢数据
- **自动继续**：正常通过自动跳过，不中断
- **按设备号分文件**：`桌面\TTAG_复测数据_{设备ID}.xlsx`，同设备测多次自动追加
- **崩溃保护**：最外层捕获所有异常，窗口不会闪退

**输出：** `桌面\TTAG_复测数据_{设备ID}.xlsx`
- Sheet1：设备信息
- Sheet2「TTAG Verify」：逐点数据（目标°C / 水浴°C / 标签°C / 误差 / 通过），绿色=通过，红色=超标
...
```matlab
% 修改 verify_plot.m 中的文件路径，直接运行
verify_plot
```
生成：精度对比图（y=x ±1°C区间）、误差散点图、误差直方图、统计摘要。

#### Python

```bash
python ttag_verify.py --device 201154  # 先跑复测，生成 Excel
# 然后用 verify_plot.m（MATLAB）或 Python matplotlib 生成图表
```

### 第六步（可选）：GUI 桌面程序（`ttag_cal_app.py`）
...
```
55 AA | Length | StationID | FuncCode | SN | TagCount | [Payload] | Checksum
 2B   | 2B LE  |   2B LE   |    1B    | 1B |    1B    |  varies   |    1B
```

### 旧协议 T100-230（每标签 9 字节）

```
Payload = 5 + 9 × N
TagBlock: RSSI(1) + Type(1) + ID(3 LE) + ADC(2 LE) + Reserved(2)
```
...
### 新协议 T100-316（每标签 8 字节）

```
Payload = 6 + 8 × N  （多 1 字节帧级 RSSI）
Header: ... TagCount(1) FrameRSSI(1)
TagBlock: Type(1) + ID(3 LE) + Temp(2 LE signed) + Reserved(2)
```
...
| +0 | 1B | 标签类型 |
| +1 | 3B | 标签 ID（小端序） |
| +4 | 2B | 温度原始值（signed int16 LE, ÷10 = °C） |
| +6 | 2B | 保留 |

- `Temp = 0xFFFF` 表示低电量
- 温度直接 = `temp_raw / 10.0`°C（无需多项式换算）
- RSSI 为帧级共享（所有标签同一信号强度）
